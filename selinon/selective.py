...
def _compute_all_combinations(items):
    """ Compute all combinations of length from 1 to len(items) """
    return chain(*map(lambda i: combinations(items, i), range(1, len(items) + 1)))


def _compute_subsequent(flow_name, node_names):
    ...
    desired_nodes_combinations = list(_compute_all_combinations(desired_nodes))

    change = True
    while change:
        change = False
        for edge_idx, edge in enumerate(Config.edge_table[flow_name]):
            if edge['from'] and edge_idx not in result and \
                    any(set(edge['from']).issubset(c) for c in desired_nodes_combinations):
...
def compute_selective_run(flow_name, task_names, follow_subflows=False, run_subsequent=False):
    ...
    result = {
        ...
        'subsequent_edges': dict.fromkeys(traversals.keys(), [])
    }

    subsequent_edges = result['subsequent_edges']
    if run_subsequent:
        if isinstance(run_subsequent, list) or isinstance(run_subsequent, tuple):
            subsequent_flows = run_subsequent
        else:
            subsequent_flows = subsequent_edges.keys()

        for flow in subsequent_flows:
            subsequent_edges[flow] = _compute_subsequent(flow, task_names)
